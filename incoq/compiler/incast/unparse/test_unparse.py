###############################################################################
# unparse.test_unparse                                                        #
# Based on Tools/parser/test_unparse.py in Python 3.3; see LICENSE.txt.       #
###############################################################################

# Overview of changes from standard 3.3 library version:
#
# - eliminated use of internal Python source tree testing facility
...
class NewFeatureTestCase(ASTTestCase):
    """Test features added since fork from standard library version."""
    
    def test_to_source(self):
        tree1 = ast.parse('a = b + c')
        tree2 = ast.parse('x = y + z')
        
        source_a = unparse.Unparser.to_source([tree1, tree2])
        
        unparse_buffer = io.StringIO()
        unparse.Unparser(tree1, unparse_buffer)
        unparse.Unparser(tree2, unparse_buffer)
        source_b = unparse_buffer.getvalue()
        if source_b.startswith('\n'):
            source_b = source_b[1:]
        
        self.assertEqual(source_a, source_b)
    
    def test_handle_op(self):
        tree1 = ast.parse('a + b').body[0].value.op
        source1 = unparse.Unparser.to_source(tree1)
        self.assertEqual(source1, '+')
        
        tree2 = ast.parse('a and b').body[0].value.op
        source2 = unparse.Unparser.to_source(tree2)
        self.assertEqual(source2, 'and')
    
    def test_unknown_node(self):
        tree = ast.parse('a + b').body[0].value
        tree.right = 5
        source = unparse.Unparser.to_source(tree)
        
        exp_source = '(a + <Unknown node "int">)'
...
class DirectoryTestCase(ASTTestCase):
    ...
    # Root directory for test dir paths.
    test_dir_root = os.path.dirname(__file__)
    # List of paths to test directories. All *.py files in these
    # directories (not recursive) will be parsed and unparsed.
    test_dirs = ['.']
    # Run on all files rather than a sample.
    run_all = True
    # Print current tested file.
    verbose = False

    def test_files(self):
        names = []
        for d in self.test_dirs:
            test_dir = os.path.join(self.test_dir_root, d)
