...
QUERYOPTIONS(
    '{(a, s, c) for a in ATTENDS for s in STUDENTS for c in COURSES if a.course == COMP101 if a.student == s if a.course == c}',
    uset_mode = 'all',
)
...
def do_query(COMP101):
    return {(a, s, c) for a in ATTENDS for s in STUDENTS for c in COURSES
                      if a.course == COMP101 if a.student == s
                      if a.course == c}

def do_query_nodemand(COMP101):
    return NODEMAND({(a, s, c) for a in ATTENDS for s in STUDENTS
                               for c in COURSES
                               if a.course == COMP101 if a.student == s
                               if a.course == c})
